...
class ComputerStoresWithProductsAdapter(
    options: FirestoreRecyclerOptions<Store>,
    context: Context
) :
    FirestoreRecyclerAdapter<Store, ComputerStoresWithProductsAdapter.StoreViewHolder>(options) {
    ...
    override fun onBindViewHolder(holder: StoreViewHolder, position: Int, model: Store) {
        ...
        if (model.getCategory() == "computers") {
